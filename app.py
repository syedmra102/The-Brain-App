...
def process_daily_submission(completed_tasks, missed_tasks, penalty_amount, penalty_confirmation, today, tasks):
    ...
    if missed_tasks == 0:
        ...
        # Missed 1 task, count the day regardless of penalty
        challenge_data['completed_days'] += 1
        challenge_data['current_day'] += 1
       
        # Only increment streak_days if penalty is paid
        if penalty_confirmation and penalty_amount > 0:
            ...
            st.session_state.submission_message = f"Day {challenge_data['current_day']-1} counted but no penalty paid, so streak not incremented. Streak: {challenge_data['streak_days']} days."
...
def setup_profile_page():
    ...
    with col1:
        if st.button("Back to Challenge Rules", use_container_width=True):
            ...
            st.rerun()
    with col2:
        if st.session_state.user_profile and st.button("Back to Daily Challenge", use_container_width=True):
            st.session_state.page = "daily_challenge"
            st.rerun()
