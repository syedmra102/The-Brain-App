...
def send_password_reset_email(to_email, reset_token):
    try:
        reset_link = f"https://your-app-domain.com/reset-password?token={reset_token}"
        
        msg = EmailMessage()
        msg['Subject'] = 'Password Reset Request - The Brain App'
...
# Email setup - Use Streamlit secrets
EMAIL_ADDRESS = st.secrets.get("email", {}).get("address", "default@email.com")
EMAIL_PASSWORD = st.secrets.get("email", {}).get("password", "default_password")
...
# Enhanced Penalty/Reward UI
def show_penalty_reward_impact(missed_tasks, savings_amount, is_perfect_day):
    """Show strong visual feedback for penalties and rewards"""
    if is_perfect_day:
        st.balloons()
        ...
        <p style='font-size: 18px;'>You're building unstoppable momentum!</p>
        </div>
        """, unsafe_allow_html=True)
    elif missed_tasks == 1 and savings_amount > 0:
        st.warning("⚠️ PENALTY PAID - DAY COUNTED")
        st.markdown(f"""
        <div style='background: linear-gradient(135deg, #FF9800, #F57C00); padding: 20px; border-radius: 10px; color: white; text-align: center;'>
        <h3>💸 PENALTY: ${savings_amount} 💸</h3>
        <p style='font-size: 16px;'>This penalty strengthens your commitment. Your savings grow!</p>
        </div>
        """, unsafe_allow_html=True)
    elif missed_tasks >= 2:
        st.error("🚫 CRITICAL FAILURE - DAY LOST")
        st.markdown("""
        <div style='background: linear-gradient(135deg, #f44336, #d32f2f); padding: 20px; border-radius: 10px; color: white; text-align: center;'>
        <h3>💀 MULTIPLE FAILURES DETECTED</h3>
        <p style='font-size: 16px;'>This day is lost. Come back stronger tomorrow!</p>
        </div>
        """, unsafe_allow_html=True)
                ...
                if not email:
                    ...
                    with st.spinner("Sending reset link..."):
                        ...
                        if user_info:
                            ...
                            try:
                                db.collection('password_resets').document(hashed_token).set(reset_data)
                                success, message = send_password_reset_email(email_clean, reset_token)
                                if success:
                                    st.success("Password reset link sent to your email")
                                    st.info("Check your email for the reset link (valid for 1 hour)")
                                else:
...
def reset_password_page():
    ...
    with col2:
        ...
        reset_doc = db.collection('password_resets').document(hashed_token).get()
        ...
        if reset_data.get('used') or reset_data.get('expires') < datetime.now():
            st.error("Reset token has expired or been used")
            st.button("Back to Sign In", use_container_width=True, on_click=lambda: st.session_state.update({"page":"signin"}))
            return
                ...
                if not new_password or not confirm_password:
                    ...
                    with st.spinner("Resetting password..."):
                        ...
                        if user_info:
                            ...
                            # Mark token as used
                            db.collection('password_resets').document(hashed_token).update({'used': True})
...
def process_daily_submission(completed_tasks, savings_amount, today, tasks):
    ...
    is_perfect_day = (missed_tasks == 0)
    
    st.session_state.show_motivational_task = True
    
    # Show enhanced penalty/reward UI
    show_penalty_reward_impact(missed_tasks, savings_amount, is_perfect_day)
