...
# --- PAGE CONFIGURATION & CUSTOM CSS (With Professional Adjustments) ---
st.set_page_config(
    ...
    if st.button("Register"):
        if new_user in st.session_state.user_db:
            ...
            st.session_state.user_db[new_user] = {
                'password': new_pass,
                'profile': {},
                'challenge': {'status': 'Pending', 'stage': None, 'day_start': None, 'daily_log': {}, 'penalty_amount': 0.0, 'badges': []}
            }
...
def predict_performance_ui():
    ...
    if st.button("Predict Performance", key="run_predict"):
        ...
        # Set text color based on background (use white for dark bars, black for light)
        text_colors = ['white' if c == '#1E90FF' else 'black' for c in colors]

        ax.bar_label(bars, labels=[f'Top {p:.1f}%' for p in percentiles], 
                     label_type='edge', padding=2, fontweight='bold', fontsize=8, color='black')
        
        ax.set_xlabel('Performance Features', fontweight='bold', fontsize=10)
        ax.set_title(f'PERFORMANCE BREAKDOWN (Predicted: Top {prediction:.1f}%)', fontweight='bold', fontsize=12, color='#000080')
        ax.set_ylabel('Performance Percentile', fontweight='bold', fontsize=10)
...
def challenge_setup_ui():
    ...
    field = st.selectbox("What is your field?", ['Programming', 'Engineering', 'Sports', 'Medicine (Doctor)', 'Art/Design', 'Other'])
    goal = st.text_input("What do you want to become? (e.g., AI Engineer, Neurosurgeon, Pro-Athlete)")
    ...
    selected_distractions = st.multiselect("Select ALL your major daily distractions:", all_distractions)

    st.header("3. Choose Your Starting Stage")
    stage_selection = st.radio("Which stage do you want to select?", list(CHALLENGE_STAGES.keys()))
...
def daily_tracking_ui():
    ...
    col3.metric("Streak Days (Penalty Paid)", challenge.get('streak_days_penalty', 0))
    ...
    checklist = {}
    for rule in rules:
        if rule != 'Fill the form daily': # Skip this for the actual form
            checklist[rule] = st.checkbox(f"✅ {rule}")
    
    st.subheader("Penalty and Pocket Money")
    penalty_input = st.number_input("If you failed any task, enter your daily pocket money/earning for the penalty (PKR):", min_value=0.0, value=0.0)
    ...
    if st.button("Save Daily Routine"):
        ...
        if tasks_skipped == 0:
            ...
            user_data['challenge']['streak_days_penalty'] = user_data['challenge'].get('streak_days_penalty', 0)
            
        elif tasks_skipped <= 2 and penalty_input > 0.0:
            # PENALTY PAID
            ...
            # DAY FAILED - More than 2 tasks skipped OR penalty not paid
            st.error(
                "❌ Day NOT Accepted! You skipped more than 2 tasks or did not pay the penalty. "
                "You must comply or pay the full penalty to save the day."
            )
            return
...
def check_stage_completion(user_data):
    ...
    if challenge['stage_days_completed'] >= current_stage_data['duration']:
        ...
        st.balloons()
        st.success(f"🌟 CONGRATULATIONS! You have completed the **{current_stage}** stage!")
