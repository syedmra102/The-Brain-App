...
st.markdown(
    ...
    .stApp {
        ...
        color: #333333; /* <--- FIXED: Dark text color for visibility */
    }
    .stButton>button {
...
def predict_performance_ui():
    ...
    if st.button("Predict Performance", key="run_predict"):
        ...
        # Set text color for labels to BLACK (THE FIX)
        ax.bar_label(bars, labels=[f'Top {p:.1f}%' for p in percentiles], 
                     label_type='edge', padding=2, fontweight='bold', fontsize=8, color='black') # <-- FIXED
        ...
        ax.tick_params(axis='x', colors='black') # <-- FIXED
        ax.tick_params(axis='y', colors='black') # <-- FIXED
...
def daily_tracking_ui():
    ...
    if st.button("Save Daily Routine"):
        ...
        if tasks_skipped == 0:
            ...
            # DAY SAVED WITH PENALTY (1 or 2 tasks skipped)
            
            # You specified that the day is NOT accepted if more than 2 tasks are skipped *even if money is paid*.
            if tasks_skipped > 2:
                st.error(
            ...
            if tasks_skipped > 0 and penalty_input == 0.0:
                 ...
                 st.error(
                    "❌ ERROR: Day NOT Accepted. You must meet all the daily challenges to save the day."
                )
            return
