# app.py
# The Brain - 105 Days (Final flow version)
# Single-file Streamlit app with local data.json storage
# Author: ChatGPT (for Syed Imran Shah)
...
def add_daily_log(username, log):
    ...
    success = log.get("work_done_hours", 0) >= required_hours and log.get("distractions_avoided", False)
    if stage == "Platinum":
        success = success and (log.get("pushups", 0) >= 30) and (log.get("water_liters", 0) >= 5)
    if stage == "Gold":
        success = success and (log.get("pushups", 0) >= 50) and (log.get("water_liters", 0) >= 5) and log.get("sugar_avoided", False) and log.get("woke_4am", False) and log.get("slept_9pm", False)
    if success:
        profile["streak_days"] = profile.get("streak_days", 0) + 1
    else:
        # add penalty money to savings and count useless day
...
def page_predict():
    st.header("Quick Prediction — Where are you globally?")
    st.markdown("Answer a few fast questions. The model gives a rough percentile (1-99).")

    # Sidebar for profile preview and distractions
    with st.sidebar:
        ...
        if st.session_state.user:
            ...
            st.markdown("---")
        st.markdown("### Distractions you face now")
        current_distractions = st.multiselect("Select current distractions", DISTRACTIONS_MASTER, default=[])
        st.markdown("---")
    ...
    exercise = st.checkbox("I exercise daily")
    water = st.number_input("Liters of water/day", 0.0, 10.0, 2.0, 0.5)
    junk = st.checkbox("I often eat junk food")
    woke4 = st.checkbox("I wake around 4:00 AM")
    sleep9 = st.checkbox("I sleep around 9:00 PM")
    if st.button("Get Prediction"):
        ...
        if pct >= 60:
            st.info(f"You're doing well — in top {pct}%. But the top 1% is achievable with discipline.")
        elif pct >= 40:
            st.info(f"You are around top {pct}%. With a plan you can climb faster.")
        else:
            st.warning(f"You're in the {pct}% range. Don't worry — start small & be consistent.")
        st.markdown("---")
...
def page_offer():
    ...
    st.markdown("Finish the stage-based plan (Silver → Platinum → Gold) and you will see results in health, discipline, skills and mindset.")
    benefits = [
        "Healthy Diet: No sugar, no alcohol, no junk food; 5L water & deep sleep",
        "Wake up early at 4 AM",
        "1 hour exercise daily",
        "Sleep early on time",
        "Deep hands-on knowledge in your field",
        "A different character with laziness removed",
        "All major distractions controlled",
        "A wealthy & investment mindset",
        "Unstoppable focus and character",
        "Positive thinking & high EQ"
    ]
...
def page_rules():
    ...
    st.markdown("""
    **Easy (Silver)** — 15 days:
    - Work 2 hours/day; avoid distractions; fill daily form.

    **Medium (Platinum)** — 30 days:
    - Work 4 hours/day; drink 5L water + 30 pushups; avoid distractions; fill daily form.

    **Hard (Gold)** — 60 days:
    - Wake 4 AM, sleep 9 PM; 1 hour morning exercise; 6 hours/day work; 5L water; no sugar; 50 pushups; positive mirror talk.
    """)
    st.write("")
    if st.button("Start Challenge (Join now)"):
        if not st.session_state.user:
            ...
            # set profile joined and started_on and set stage to Silver
...
def page_profile():
    if not st.session_state.user:
        st.error("Login first to see profile.")
        return
    username = st.session_state.user
    prof = store["users"][username]["profile"]
    st.header("Your Profile (view & edit)")
    ...
    if save:
        update_profile(username, {"field": field, "interests": interests, "hours_per_day": hours, "stage": stage})
        st.success("Profile updated.")
        # after saving show summary and button to go to daily routine
        st.session_state.page = "profile"  # stay here to show updated data
        st.rerun()

    st.markdown("---")
    st.markdown("**Profile Summary**")
    prof = store["users"][username]["profile"]  # reload
    ...
    st.write(f"- Useless days: {prof.get('useless_days',0)}")
    st.write("")
    if st.button("Go to Daily Routine"):
...
def page_daily():
    ...
    with st.form("daily_form"):
        ...
        pocket_money = st.number_input("If you fail, pocket money to save today (PKR)", 0.0, 10000.0, 0.0, 1.0)
    ...
    if submit:
        ...
        # Evaluate success message
        required_hours = 2 if stage == "Silver" else (4 if stage == "Platinum" else 6)
        success = (log["work_done_hours"] >= required_hours) and log["distractions_avoided"]
        if stage == "Platinum":
        ...
        if success:
            st.success("Great! Today's tasks completed. Take a motivational quote image and set it as your wallpaper.")
        else:
    ...
    # Show short checklist style of last logs
    df = pd.DataFrame(user_logs(username))
    if not df.empty:
        st.markdown("---")
        st.subheader("Recent (top 10)")
...
def main():
    ...
    # Sidebar navigation & profile quick view
    ...
    if page == "home":
        ...
        st.markdown("<p style='color:#eaf6ff;'>Login / Register to start. The flow: Prediction → Offer → Rules → Profile → Daily Routine.</p>", unsafe_allow_html=True)
        page_login()
    elif page == "predict":
        page_predict()
    elif page == "offer":
